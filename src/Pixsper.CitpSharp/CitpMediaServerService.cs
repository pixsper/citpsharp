...
namespace Pixsper.CitpSharp
{
	...
	public class CitpMediaServerService : CitpServerService
	{
			...
			TcpServerConnection client)
		{
			...
			Logger.LogInformation("{Client}: Get element library information packet received", client);
			...
			{
				Logger.LogWarning("{Client}: Requested element library information for libraries with a parent library ID which can have no child elements", client);
			}
			else
			{
			...
			TcpServerConnection client)
		{
			...
			Logger.LogInformation("{Client}: Get element information packet received", client);

			if (!_device.ElementLibraries.TryGetValue(packet.LibraryId, out var library))
			{
				Logger.LogWarning("{Client}: Requested non-existant library - {PacketLibraryId}", client, packet.LibraryId);
				return;
			}
			...
			switch (library.LibraryType)
			{
				...
				default:
					...
					{
						Logger.LogWarning("{Client}: Requested unsupported library type for MSEX V1.0", client);
						return;
					}
			...
			TcpServerConnection client)
		{
			...
			Logger.LogInformation("{Client}: Get element library thumbnail packet received", client);
			...
			foreach (var id in libraryIds)
			{
				...
				{
					Logger.LogWarning("{Client}: Requested thumbnail for non-existant library {Id}", client, id);
					continue;
				}

				if (library.LibraryType != packet.LibraryType)
				{
					Logger.LogWarning("{Client}: Requested thumbnail for library {Id} with expected type {PacketLibraryType}, actual type is {LibraryLibraryType}", client, id, packet.LibraryType, library.LibraryType);
					continue;
				}
				...
				if (image == null)
				{
					Logger.LogWarning("{Client}: Failed to get thumbnail for library {Id}", client, id);
					continue;
				}
			...
			TcpServerConnection client)
		{
			...
			Logger.LogInformation("{Client}: Get element thumbnail packet received", client);

			if (!_device.ElementLibraries.TryGetValue(packet.LibraryId, out var library))
			{
				Logger.LogWarning("{Client}: Requested thumbnails for elements in non-existant library {PacketLibraryId}", client, packet.LibraryId);
				return;
			}
			...
			{
				if (client != null)
					Logger.LogWarning("{Client}: Requested thumbnails for elements in library {PacketLibraryId} with expected type {PacketLibraryType}, actual type is {LibraryLibraryType}", client, packet.LibraryId, packet.LibraryType, library.LibraryType);
				return;
			}
			...
			foreach (byte i in elementNumbers)
			{
				...
				{
					Logger.LogWarning("{Client}: Requested thumbnail for non-existant element {Index} in library {PacketLibraryId}", client, i, packet.LibraryId);
					continue;
				}
				...
				if (image == null)
				{
					Logger.LogWarning("{Client}: Failed to get thumbnail for element {Index} in library {PacketLibraryId}", client, i, packet.LibraryId);
					continue;
				}
