...
namespace Pixsper.CitpSharp
{
		...
		public abstract class CitpServerService : CitpService
	{
		...
		{
			Logger.LogInformation($"{client}: Client information packet received");
		...
		internal virtual void OnGetVideoSourcesPacketReceived(GetVideoSourcesPacket packet, TcpServerConnection client)
		{
			...
			Logger.LogInformation($"{client}: Get video sources packet received");
		...
		internal virtual void OnRequestStreamPacketReceived(RequestStreamPacket packet, TcpServerConnection client)
		{
			...
			Logger.LogInformation($"{client}: Requested stream ID {packet.SourceId} @ {packet.FrameWidth}x{packet.FrameHeight}, {packet.Fps} fps, {packet.FrameFormat.GetCustomAttribute<CitpId>().IdString}, timeout {packet.Timeout} sec");
		...
		{
			Logger.LogInformation($"{client}: Nack sent for message type {packet.MessageType}");
